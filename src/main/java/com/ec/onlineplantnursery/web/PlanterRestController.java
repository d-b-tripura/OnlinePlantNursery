...
public class PlanterRestController {
	...
	@ApiOperation(value = "planter get mapping to view planters by name", response = PlanterResponseDto.class)
	@GetMapping("/search/{text}")
	public List<PlanterResponseDto> searchProduct(@PathVariable String text) throws ResourceNotFoundException {

		log.info("inside Get all planters by product name");

		return planterService.searchProduct(text).stream()
				.map(planter -> modelMapper.map(planter, PlanterResponseDto.class)).collect(Collectors.toList());
